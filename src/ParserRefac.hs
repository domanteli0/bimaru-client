...
module ParserRefac(parse, Token(..), test0, test1, tokenizeYaml) where
...
tokenizeYaml str = pipeline str
    where
        pipeline str' = (splitDiff . diff . normalizeWhitespace . parseScalarToken . joinBetweenScalar . joinUnknown . tokenizeYaml') (str' ++ "\n") -- ++[TokenEndOfDocument]
        tokenizeYaml' :: String -> [Token]
        tokenizeYaml' "" = []
        tokenizeYaml' str' =  do
    ...
    where
        tokenP' t = Parser func
            where
                ...
                func _ = Left "ERR_02: Empty token list"
...
parseYaml = parseList <|> parseNull <|> parseNumber <|> parseString <|> parseIndented

parseNull :: Parser Document
parseNull = const DNull <$> tokenP TokenScalarNull

parseNumber :: Parser Document
parseNumber = func <$> tokenListP [TokenScalarInt 0]
    where
        func :: [Token] -> Document
...
parseString = func <$> tokenListP [TokenScalarString ""]
    where
        func :: [Token] -> Document
...
-- new line and whitespace skip
wsnlOptional :: Parser [Token]
-- wsnlOptional = wsOptional *> nlOptional
wsnlOptional = wsOptional <|> nlOptional

end :: Parser [Token]
end = (nlOptional *> tokenListP [TokenEndOfDocument]) <|> tokenListP [TokenNewLine]

parseList :: Parser Document
parseList = DList <$> some one
    where
        one = tokenP TokenDashListItem *> wsOptional *> parseYaml <* wsOptional <* end
        -- one = tokenP TokenDashListItem *> parseYaml <* end
        -- element = 

parseIndented :: Parser Document
parseIndented = nlOptional *> tokenP (TokenSpaceDiff 0) *> parseYaml
-- parseIndented = do
--     (TokenSpaceDiff ind) <- tokenP (TokenSpaceDiff 0)
--     doc <- parseYaml
--     (_) <- tokenPExact (TokenSpaceDiff ind)

--     return (tokenP (TokenSpaceDiff 0) *> parseYaml <* tokenP (TokenSpaceDiff 0))
